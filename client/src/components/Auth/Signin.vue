         ...
         <v-container>
           <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleSigninUser">
             ...
             <v-layout row>
              ...
              <v-layout row>
               <v-flex xs12>
                 <v-text-field :rules="passwordRules" v-model="password" prepend-icon="extension" label="Password" type="text" required>
                 </v-text-field>
               </v-flex>
             </v-layout>
              ...
              <v-layout row>
               <v-flex xs12>
                 <v-btn :disabled="!isFormValid || loading" color="accent" type="submit">Signin</v-btn>
  ...
  data() {
    return {
      ...
      passwordRules: [
        ...
        password => password.length < 10 || 'Password must be less than 7 characters'
      ]
    }
  },
  computed: {
